...
function filterProducts() {
    ...
    // Use binary search to determine the cutoff index
    const index = binarySearch(allProducts, maxPrice);
    
    // Get the products up to the cutoff index
    filteredProducts = index === -1 ? [] : allProducts.slice(0, index + 1);
...
function binarySearch(products, maxPrice) {
    let low = 0, high = products.length - 1, result = -1;
    while (low <= high) {
        const mid = Math.floor((low + high) / 2);
        if (parseInt(products[mid].store_price, 10) <= maxPrice) {
